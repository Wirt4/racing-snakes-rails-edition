import { ColorName } from './color_name';

interface HSLInterface {
	hue: number;
	saturation: number;
	lightness: number;
}
/**
 * hue is a number between 0 and 360
 * saturation and lightness are numbers between 0 and 100
 */

function nameToProfile(color_label: ColorName): HSLInterface {
	switch (color_label) {
		case ColorName.YELLOW:
			return { hue: 70, saturation: 100, lightness: 65 };
		case ColorName.BLUE:
			return { hue: 200, saturation: 100, lightness: 55 }
		case ColorName.RED:
			return { hue: 0, saturation: 100, lightness: 60 };
		case ColorName.GREEN:
			return { hue: 120, saturation: 100, lightness: 50 };
		case ColorName.WHITE:
			return { hue: 330, saturation: 100, lightness: 55 };
		case ColorName.BLACK:
			return { hue: 270, saturation: 40, lightness: 8 };
		case ColorName.PURPLE:
			return { hue: 280, saturation: 100, lightness: 60 };
		case ColorName.ORANGE:
			return { hue: 20, saturation: 100, lightness: 60 };
		default:
			throw new Error(`Unknown color label: ${color_label}`);
	}
}

export { HSLInterface, nameToProfile };
