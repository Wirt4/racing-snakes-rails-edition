...
describe('Arena Tests', () => {
    ...
    test('should have correct boundary wall coordinates', () => {
        expect(arena.walls)
            .toContainEqual(expect.objectContaining({ line: { start: { x: 0, y: 0 }, end: { x: 0, y: 10 } } }));
        expect(arena.walls)
            .toContainEqual(expect.objectContaining({ line: { start: { x: 0, y: 0 }, end: { x: 10, y: 0 } } }));
        expect(arena.walls)
            .toContainEqual(expect.objectContaining({ line: { start: { x: 10, y: 0 }, end: { x: 10, y: 10 } } }));
        expect(arena.walls)
            .toContainEqual(expect.objectContaining({ line: { start: { x: 0, y: 10 }, end: { x: 10, y: 10 } } }));
    });
});
...
function containsExactAmount(gridLines, lambda, targetTotal) {
    let matchedCount = 0;
