...
onmessage = (e) => {
    ...
    if (msg.type === "init") {
        ...
        batchRenderer = new BatchRenderer(new Renderer(ctx), msg.settings.CANVAS_WIDTH, msg.settings.CANVAS_HEIGHT, ColorName.BLUE);
        ...
        const arena = new Arena(arenaSize, msg.settings.GRID_CELL_SIZE, ColorName.BLACK);
        const map = new GameMap(arena, player);
        raycaster = new Raycaster(msg.settings.RESOLUTION, msg.settings.FIELD_OF_VISION, msg.settings.CANVAS_WIDTH, msg.settings.CANVAS_HEIGHT, msg.settings.MAX_DISTANCE, msg.settings.HORIZON_LINE_RATIO * msg.settings.CANVAS_HEIGHT, msg.settings.WALL_HEIGHT, msg.settings.CAMERA_HEIGHT);
